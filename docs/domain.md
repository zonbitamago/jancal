...
`Hand` モデルに `openMentsu` フィールドを追加し、副露面子を保持可能にした。
`analyzeHandWithOpen()` で副露面子と閉じた手牌を分離して分解する。

`Problem` モデルに `openGroups` フィールドを追加し、UI上で副露グループを
オレンジの下線で視覚的に区別する。

食い下がりの翻数減少は `yaku_judge.dart` で `hand.isMenzen` フラグにより自動判定。

## 動的問題生成

`ProblemGenerator` クラス（`lib/domain/problem_generator.dart`）により、
ドメイン層のパイプラインを活用して問題を無限に生成できる。

- レベルに応じた手牌パターン（ピンフ、タンヤオ、七対子、混一色、清一色等）
- 自動的に役判定→符計算→点数計算を実行し、正解と選択肢を生成
- 各レベルの翻数範囲に収まることを検証
